...
def analyze_wav_interval(path: str | Path, start: float | None, end: float | None) -> dict[str, float | None]:
    source = Path(path)
    ...
    block_rms = [_rms(samples[index : index + block_size]) for index in range(0, len(samples), block_size)]
