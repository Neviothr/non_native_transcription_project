...
def _transcript_rows(project: ProjectData) -> list[list[Any]]:
    headers = [
        ...
        "Speaker", "Raw Speaker", "Zoom Transcript", "ChatGPT Transcript", "Whisper Transcript",
        "ML Enhanced Transcript", "Enhancement Source", "Enhancement Confidence", "Enhancement Method",
        "Final Transcript", "Gold Standard", "Whisper Confidence", "Agreement", "Quality Score", "Quality Classification",
    ...
    for turn in project.turns:
        rows.append([
            ...
            turn.model_text,
            turn.enhanced_text,
            turn.enhancement_source,
            turn.enhancement_confidence,
            turn.enhancement_method,
            turn.final_text,
            turn.gold_text,
...
def _model_rows(project: ProjectData) -> list[list[Any]]:
    rows: list[list[Any]] = [["Task", "Model", "Accuracy", "Macro F1"]]
...
def export_xlsx(project: ProjectData, path: str | Path) -> Path:
    ...
    sheets = [
        ("Transcript", _transcript_rows(project), [14, 10, 18, 8, 11, 11, 16, 16, 35, 35, 38, 40, 18, 18, 20, 42, 38, 16, 12, 12, 24, 12, 18, 15, 14, 16, 18, 17, 14, 16, 30], {14, 19, 20}),
        ("Evaluation", _metric_rows(project), [42, 22], set()),
        ("Source Comparison", _comparison_rows(project), [24, 20, 22], {2, 3}),
        ("ML Model Comparison", _model_rows(project), [32, 28, 18, 18], {3, 4}),
